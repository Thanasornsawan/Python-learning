...
#if unknow value > middle value -> result go to the right of charset
#if unknow value < middle value -> result go to the left of charset
#binary search help find possible value within limit total queries which automate tool can't do
def extract_hash_bst(charset, user_id, password_length):
        found = ""
        for index in range(0, password_length):
                start = 0
                end = len(charset-1)
                #check to ensure that still have middle value
                while start <= end:
                        if end - start == 1:
